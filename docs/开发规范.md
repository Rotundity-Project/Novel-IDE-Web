...
- 规格目录：`docs/specs/`
- 模板目录：`docs/spec-kit/templates/`
- 命名：`0001-<slug>.md`（递增编号 + 小写短横线）
- 状态：Draft / Reviewed / Accepted / Implemented / Deprecated
- 最小要求：目标/约束/验收标准/验证方式/实现落点（Implemented 后必须补齐链接）
- 变更要求：当 PR 改动了用户可见行为、接口契约、错误码、迁移策略时，必须同步更新对应 spec 或补 ADR。
