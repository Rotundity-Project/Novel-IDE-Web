# Novel-IDE-Web 智能体系统设计文档

## 1. 系统概述

### 1.1 设计目标
- 提供灵活的 AI 助手定制能力
- 支持多种智能体类型（内置、用户自定义、社区分享）
- 可配置的行为参数和知识领域
- 良好的扩展性和可维护性

### 1.2 核心概念

#### 智能体 (Agent)
智能体是具有特定写作风格、知识领域和行为模式的 AI 助手，通过系统提示词和行为参数配置其特性。

#### 系统提示词 (System Prompt)
定义智能体的角色、行为准则和知识范围的文本指令。

#### 行为参数 (Behavior Parameters)
控制智能体输出特性的数值参数（创意度、严谨度、细节度等）。

#### 上下文注入 (Context Injection)
将作品相关信息（角色、世界观、前文）注入到 AI 请求中。

---

## 2. 智能体数据模型

### 2.1 核心数据结构

```typescript
interface Agent {
  // 基本信息
  id: string;                    // UUID
  userId?: string;               // 创建者，null 表示内置智能体
  name: string;                  // 智能体名称
  description: string;           // 描述
  avatarUrl?: string;            // 头像

  // 分类
  category: AgentCategory;       // 分类
  subcategory?: string;         // 子分类

  // 配置
  systemPrompt: string;          // 系统提示词
  behavior: AgentBehavior;       // 行为参数
  knowledge: string[];          // 知识领域
  examples?: string[];          // 示例对话

  // 模型偏好
  modelPreference?: string;     // 首选模型 ID

  // 权限
  isBuiltin: boolean;           // 是否内置
  isPublic: boolean;            // 是否公开
  isOfficial: boolean;          // 是否官方推荐

  // 统计
  usageCount: number;           // 使用次数
  rating: number;               // 评分 (0-5)
  ratingCount: number;          // 评分人数

  // 分享
  shareCode?: string;           // 分享码
  sharedAt?: Date;              // 分享时间

  // 时间
  createdAt: Date;
  updatedAt: Date;
  deletedAt?: Date;
}

type AgentCategory =
  | 'builtin'      // 内置智能体
  | 'genre'        // 类型助手（科幻、玄幻、都市等）
  | 'task'         // 任务助手（续写、改写、校对等）
  | 'character'     // 角色助手（对话生成等）
  | 'custom';      // 用户自定义

interface AgentBehavior {
  creativity: number;    // 创意度 (0-1)
  formality: number;    // 严谨度 (0-1)
  detailLevel: number;  // 细节度 (0-1)
  emotionLevel: number; // 情感度 (0-1)
  pacing: 'slow' | 'normal' | 'fast'; // 节奏
  style?: string;       // 风格标签
}
```

### 2.2 数据库表结构

```sql
CREATE TABLE agents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,

  name VARCHAR(100) NOT NULL,
  description TEXT,
  avatar_url VARCHAR(500),

  category VARCHAR(50) NOT NULL,
  subcategory VARCHAR(100),

  system_prompt TEXT NOT NULL,
  behavior JSONB NOT NULL,
  knowledge TEXT[],
  examples TEXT[],

  model_preference VARCHAR(100),

  is_builtin BOOLEAN DEFAULT FALSE,
  is_public BOOLEAN DEFAULT FALSE,
  is_official BOOLEAN DEFAULT FALSE,

  usage_count INTEGER DEFAULT 0,
  rating DECIMAL(2,1) DEFAULT 0,
  rating_count INTEGER DEFAULT 0,

  share_code VARCHAR(20) UNIQUE,
  shared_at TIMESTAMP,

  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  deleted_at TIMESTAMP
);
```

---

## 3. 内置智能体设计

### 3.1 通用助手

#### 写作助手
```json
{
  "name": "写作助手",
  "description": "通用的小说写作助手，能够协助续写、改写、润色",
  "category": "builtin",
  "systemPrompt": "你是一个专业的小说写作助手。你擅长续写、改写和润色小说内容。在写作时，保持连贯性，注重人物塑造和情节发展。你的文笔流畅，用词精准，能够适应多种文学风格。",
  "behavior": {
    "creativity": 0.7,
    "formality": 0.6,
    "detailLevel": 0.7,
    "emotionLevel": 0.6,
    "pacing": "normal"
  }
}
```

#### 校对编辑
```json
{
  "name": "校对编辑",
  "description": "专注于语法、标点、错别字和逻辑一致性的校对",
  "category": "task",
  "systemPrompt": "你是一个专业的校对编辑。你的任务是检查文本中的语法错误、标点错误、错别字、逻辑矛盾等问题。修改时要保持原文风格，只修正错误，不进行创造性改写。",
  "behavior": {
    "creativity": 0.2,
    "formality": 0.9,
    "detailLevel": 0.9,
    "emotionLevel": 0.1,
    "pacing": "slow"
  }
}
```

### 3.2 类型助手

#### 科幻小说专家
```json
{
  "name": "科幻小说专家",
  "description": "专注于科幻小说创作，擅长未来科技、宇宙探索等题材",
  "category": "genre",
  "systemPrompt": "你是一个专业的科幻小说创作专家。你精通未来科技、宇宙探索、人工智能等科幻元素。在写作时，注重科学合理性和想象力，创造宏大的世界观和惊险的情节。你善于描写未来社会的各种细节，构建充满想象力的未来世界。",
  "behavior": {
    "creativity": 0.9,
    "formality": 0.7,
    "detailLevel": 0.8,
    "emotionLevel": 0.5,
    "pacing": "normal"
  },
  "knowledge": ["科学知识", "未来科技", "宇宙探索", "人工智能"]
}
```

#### 古风小说专家
```json
{
  "name": "古风小说专家",
  "description": "擅长古代背景的小说创作，熟悉古代文化和语言风格",
  "category": "genre",
  "systemPrompt": "你是一个专业的古风小说创作专家。你精通古代历史、文化、礼仪，擅长运用古风语言和典故。在写作时，注重时代背景的真实性和语言的古典韵味，营造浓郁的古风氛围。你的人物塑造贴合时代特征，情节设计富有古典韵味。",
  "behavior": {
    "creativity": 0.7,
    "formality": 0.8,
    "detailLevel": 0.8,
    "emotionLevel": 0.7,
    "pacing": "slow"
  },
  "knowledge": ["古代历史", "古典文学", "诗词典故", "古代礼仪"]
}
```

#### 悬疑小说专家
```json
{
  "name": "悬疑小说专家",
  "description": "擅长悬念设置、谜题设计和推理逻辑",
  "category": "genre",
  "systemPrompt": "你是一个专业的悬疑小说创作专家。你擅长设置悬念、设计谜题、构建合理的推理逻辑。在写作时，注重情节的紧张感和伏笔的埋设，合理控制信息披露的节奏，让读者始终保持好奇心。你的故事逻辑严密，结局出人意料又合情合理。",
  "behavior": {
    "creativity": 0.8,
    "formality": 0.6,
    "detailLevel": 0.6,
    "emotionLevel": 0.6,
    "pacing": "fast"
  },
  "knowledge": ["推理逻辑", "犯罪心理学", "法律常识"]
}
```

### 3.3 任务助手

#### 对话生成助手
```json
{
  "name": "对话生成助手",
  "description": "专注于角色对话的生成和优化",
  "category": "task",
  "systemPrompt": "你是一个专业的对话生成助手。你擅长根据角色性格和情境生成自然、生动的对话。在生成对话时，注重语言的个性化，每个角色的说话方式都应该符合其性格特征。对话要有节奏感，避免冗长，适当运用潜台词和肢体语言描写。",
  "behavior": {
    "creativity": 0.6,
    "formality": 0.5,
    "detailLevel": 0.7,
    "emotionLevel": 0.9,
    "pacing": "normal"
  }
}
```

#### 描写增强助手
```json
{
  "name": "描写增强助手",
  "description": "擅长环境、动作、心理等描写",
  "category": "task",
  "systemPrompt": "你是一个专业的描写增强助手。你擅长环境描写、动作描写、心理描写等各类描写。在描写时，运用多种感官（视觉、听觉、嗅觉、触觉），注重细节和比喻的运用，让描写更加生动、形象。同时注意描写的节奏，避免过度描写拖慢情节。",
  "behavior": {
    "creativity": 0.7,
    "formality": 0.6,
    "detailLevel": 0.9,
    "emotionLevel": 0.7,
    "pacing": "normal"
  }
}
```

---

## 4. 智能体工作流程

### 4.1 请求处理流程

```
用户发起 AI 请求
       ↓
   选择智能体
       ↓
   加载智能体配置
       ↓
   构建提示词
       ↓
   注入上下文
       ↓
   调用 LLM API
       ↓
   流式返回结果
       ↓
   更新统计
```

### 4.2 提示词构建

#### 模板系统
```typescript
interface PromptTemplate {
  system: string;
  context?: string;
  user: string;
}

function buildPrompt(agent: Agent, context: Context, userInput: string): PromptTemplate {
  // 1. 系统提示词（基础）
  let systemPrompt = agent.systemPrompt;

  // 2. 注入行为参数
  systemPrompt += `\n\n【行为参数】\n创意度: ${agent.behavior.creativity}\n严谨度: ${agent.behavior.formality}\n细节度: ${agent.behavior.detailLevel}`;

  // 3. 构建上下文
  const contextInfo = buildContext(context, agent);

  // 4. 用户输入
  const userPrompt = userInput;

  return {
    system: systemPrompt,
    context: contextInfo,
    user: userPrompt
  };
}
```

#### 上下文构建
```typescript
interface Context {
  workId: string;
  chapterId?: string;
  includeCharacters?: boolean;
  includeWorldview?: boolean;
  includePrevious?: boolean;
  previousLength?: number;
}

function buildContext(context: Context, agent: Agent): string {
  const parts: string[] = [];

  // 作品信息
  const work = await getWork(context.workId);
  parts.push(`【作品信息】\n标题: ${work.title}\n类型: ${work.genre}\n描述: ${work.description}`);

  // 角色信息
  if (context.includeCharacters) {
    const characters = await getCharacters(context.workId);
    parts.push(`【角色信息】\n${characters.map(c => `${c.name}: ${c.description}`).join('\n')}`);
  }

  // 世界观信息
  if (context.includeWorldview) {
    const worldview = await getWorldview(context.workId);
    parts.push(`【世界观设定】\n${worldview}`);
  }

  // 前文
  if (context.includePrevious && context.chapterId) {
    const chapter = await getChapter(context.chapterId);
    const previousContent = chapter.content.slice(-context.previousLength || 1000);
    parts.push(`【前文】\n${previousContent}`);
  }

  return parts.join('\n\n');
}
```

### 4.3 行为参数应用

#### 温度控制映射
```typescript
function mapBehaviorToTemperature(behavior: AgentBehavior): number {
  // 创意度越高，温度越高
  // 创意度: 0-1 → 温度: 0.3-1.2
  const temperature = 0.3 + behavior.creativity * 0.9;

  // 严谨度高时，适当降低温度
  const formalityAdjustment = behavior.formality > 0.7 ? -0.1 : 0;

  return Math.max(0.1, Math.min(1.5, temperature + formalityAdjustment));
}
```

#### 输出长度控制
```typescript
function mapBehaviorToMaxLength(behavior: AgentBehavior): number {
  // 细节度越高，允许输出越长
  // 细节度: 0-1 → 最大 tokens: 500-2000
  return Math.floor(500 + behavior.detailLevel * 1500);
}
```

---

## 5. 智能体管理

### 5.1 创建智能体

#### 前端界面
```
┌─────────────────────────────────────────────┐
│  创建智能体                                 │
├─────────────────────────────────────────────┤
│                                             │
│  基本信息                                   │
│  名称: [输入智能体名称...]                 │
│  描述: [输入描述...]                       │
│  分类: [自定义 ▼] [子分类 ▼]             │
│                                             │
│  系统提示词                                 │
│  ┌─────────────────────────────────────┐   │
│  │ 你是一个专业的写作助手...          │   │
│  │                                    │   │
│  │                                    │   │
│  └─────────────────────────────────────┘   │
│  [插入角色] [插入知识] [模板选择]          │
│                                             │
│  行为参数                                   │
│  创意度: [━━━━━━━━●━] 0.7                │
│  严谨度: [━━━━━●━━━━━] 0.5                │
│  细节度: [━━━━━━●━━━━] 0.6                │
│  情感度: [━━━━━━━●━━] 0.7                │
│                                             │
│  知识领域                                   │
│  [+ 添加知识领域]                          │
│  [✕ 科幻] [✕ 未来科技] [✕ 宇宙探索]       │
│                                             │
│  示例对话（可选）                           │
│  ┌─────────────────────────────────────┐   │
│  │ 用户: 请写一段战斗场景              │   │
│  │ AI: 刀锋划过空气...                │   │
│  │                                    │   │
│  └─────────────────────────────────────┘   │
│                                             │
│  模型偏好                                   │
│  [自动选择 ▼]                              │
│                                             │
│             [创建] [预览] [取消]            │
│                                             │
└─────────────────────────────────────────────┘
```

#### 后端 API
```typescript
POST /api/v1/agents
```

### 5.2 编辑智能体

#### 版本历史
```typescript
interface AgentVersion {
  id: string;
  agentId: string;
  version: number;
  systemPrompt: string;
  behavior: AgentBehavior;
  createdAt: Date;
}

// 支持智能体配置的版本历史和回滚
```

### 5.3 智能体预览

#### 测试对话
```typescript
interface AgentPreview {
  agentId: string;
  testMessages: Message[];
  response: string;
}
```

用户可以在保存前测试智能体效果。

### 5.4 智能体分享

#### 分享机制
```typescript
interface AgentShare {
  shareCode: string;      // 分享码（8位随机）
  shareUrl: string;      // 分享链接
  expiresAt?: Date;      // 过期时间（可选）
}

// 生成分享码
function generateShareCode(): string {
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  let code = '';
  for (let i = 0; i < 8; i++) {
    code += chars[Math.floor(Math.random() * chars.length)];
  }
  return code;
}
```

#### 导入智能体
```typescript
POST /api/v1/agents/import
{
  "shareCode": "ABC123XY"
}
```

---

## 6. 智能体评分与推荐

### 6.1 评分系统

#### 评分标准
```typescript
interface AgentRating {
  agentId: string;
  userId: string;
  score: number;      // 1-5 星
  comment?: string;   // 评价内容
  tags?: string[];    // 评价标签
  createdAt: Date;
}

// 计算综合评分
function calculateOverallRating(ratings: AgentRating[]): number {
  if (ratings.length === 0) return 0;

  const sum = ratings.reduce((acc, r) => acc + r.score, 0);
  const avg = sum / ratings.length;

  // 使用贝叶斯平均，平衡评分人数少的情况
  const globalAvg = 3.5;
  const minRatings = 10;
  const bayesian = (sum + globalAvg * minRatings) / (ratings.length + minRatings);

  return Math.round(bayesian * 10) / 10;
}
```

### 6.2 推荐系统

#### 基于用户行为的推荐
```typescript
interface UserAgentPreference {
  userId: string;
  agentId: string;
  usageCount: number;
  lastUsedAt: Date;
  avgRating?: number;
}

// 推荐算法
function recommendAgents(userId: string): Agent[] {
  // 1. 获取用户历史使用数据
  const preferences = getUserPreferences(userId);

  // 2. 分析用户偏好
  const preferredCategories = analyzePreferences(preferences);
  const preferredBehavior = analyzeBehavior(preferences);

  // 3. 排序候选智能体
  const candidates = getCandidateAgents(userId);

  // 4. 计算匹配分数
  const scored = candidates.map(agent => ({
    agent,
    score: calculateMatchScore(agent, preferredCategories, preferredBehavior)
  }));

  // 5. 返回 Top N
  return scored
    .sort((a, b) => b.score - a.score)
    .slice(0, 10)
    .map(s => s.agent);
}

function calculateMatchScore(
  agent: Agent,
  categories: string[],
  behavior: AgentBehavior
): number {
  let score = 0;

  // 类别匹配
  if (categories.includes(agent.category)) {
    score += 30;
  }

  // 行为匹配
  const behaviorDiff = Math.abs(agent.behavior.creativity - behavior.creativity) +
                      Math.abs(agent.behavior.formality - behavior.formality) +
                      Math.abs(agent.behavior.detailLevel - behavior.detailLevel);
  score += Math.max(0, 40 - behaviorDiff * 20);

  // 评分
  score += agent.rating * 10;

  // 使用热度
  score += Math.log10(agent.usageCount + 1) * 5;

  return score;
}
```

---

## 7. 智能体模板系统

### 7.1 预设模板

#### 模板结构
```typescript
interface AgentTemplate {
  id: string;
  name: string;
  description: string;
  category: AgentCategory;
  systemPromptTemplate: string;  // 带变量的提示词模板
  defaultBehavior: AgentBehavior;
  variables: TemplateVariable[];
}

interface TemplateVariable {
  key: string;
  label: string;
  type: 'text' | 'textarea' | 'select' | 'multiselect';
  required: boolean;
  options?: string[];
  placeholder?: string;
}
```

#### 示例模板

```typescript
const genreExpertTemplate: AgentTemplate = {
  id: 'template-genre-expert',
  name: '类型专家模板',
  description: '用于创建特定类型的写作助手',
  category: 'genre',
  systemPromptTemplate: `你是一个专业的{genreName}创作专家。你擅长{expertise}。在写作时，{writingStyle}。你的{characteristic}。`,
  defaultBehavior: {
    creativity: 0.7,
    formality: 0.6,
    detailLevel: 0.7,
    emotionLevel: 0.6,
    pacing: 'normal'
  },
  variables: [
    {
      key: 'genreName',
      label: '类型名称',
      type: 'text',
      required: true,
      placeholder: '例如：科幻、玄幻、武侠'
    },
    {
      key: 'expertise',
      label: '擅长领域',
      type: 'textarea',
      required: true,
      placeholder: '描述该智能体擅长的内容'
    },
    {
      key: 'writingStyle',
      label: '写作风格',
      type: 'textarea',
      required: true
    },
    {
      key: 'characteristic',
      label: '特点描述',
      type: 'textarea',
      required: true
    }
  ]
};
```

### 7.2 模板使用

#### 前端界面
```
┌─────────────────────────────────────────────┐
│  从模板创建智能体                           │
├─────────────────────────────────────────────┤
│                                             │
│  选择模板                                   │
│  ┌─────────────────────────────────────┐   │
│  │ 📋 类型专家模板                     │   │
│  │ 用于创建特定类型的写作助手          │   │
│  └─────────────────────────────────────┘   │
│  ┌─────────────────────────────────────┐   │
│  │ 📋 任务助手模板                     │   │
│  │ 用于创建特定任务的助手              │   │
│  └─────────────────────────────────────┘   │
│  ┌─────────────────────────────────────┐   │
│  │ 📋 角色模拟模板                     │   │
│  │ 用于模拟特定角色的对话              │   │
│  └─────────────────────────────────────┘   │
│                                             │
│  ─────────────────────────────────          │
│  或从空白开始创建                           │
│                                             │
└─────────────────────────────────────────────┘
```

---

## 8. 智能体市场（未来）

### 8.1 市场功能

#### 社区智能体
- 用户可公开分享自己的智能体
- 浏览、搜索、下载社区智能体
- 评分和评论
- 收藏和推荐

#### 智能体分类
- 按类型分类
- 按用途分类
- 按热度排序
- 按评分排序

### 8.2 官方智能体

#### 精选推荐
- 官方审核的高质量智能体
- 定期更新和优化
- 专业写作顾问参与设计

#### 定制服务（付费）
- 根据用户需求定制智能体
- 专业写作团队提供服务

---

## 9. 性能优化

### 9.1 缓存策略

```typescript
// 智能体配置缓存
const agentCache = new Map<string, Agent>();

async function getAgent(agentId: string): Promise<Agent> {
  // 先查缓存
  if (agentCache.has(agentId)) {
    return agentCache.get(agentId)!;
  }

  // 查数据库
  const agent = await db.agents.findUnique({ where: { id: agentId } });

  // 存入缓存
  agentCache.set(agentId, agent);

  // 定时过期
  setTimeout(() => {
    agentCache.delete(agentId);
  }, 3600000); // 1 小时

  return agent;
}
```

### 9.2 预编译提示词

```typescript
// 对内置智能体的提示词进行预编译
const compiledPrompts = new Map<string, CompiledPrompt>();

interface CompiledPrompt {
  systemPrompt: string;
  behavior: AgentBehavior;
}

async function compileAgentPrompt(agent: Agent): Promise<CompiledPrompt> {
  // 预处理系统提示词
  const systemPrompt = preprocessSystemPrompt(agent.systemPrompt);

  return {
    systemPrompt,
    behavior: agent.behavior
  };
}
```

---

## 10. 扩展性设计

### 10.1 插件系统（未来）

```typescript
interface AgentPlugin {
  id: string;
  name: string;
  description: string;
  author: string;

  // 插件钩子
  onBeforeGenerate?: (context: GenerationContext) => Promise<void>;
  onAfterGenerate?: (result: GenerationResult) => Promise<void>;
  onPromptBuild?: (prompt: PromptTemplate) => Promise<PromptTemplate>;

  // 插件配置
  config?: Record<string, any>;
}
```

### 10.2 自定义函数调用（未来）

```typescript
interface AgentFunction {
  id: string;
  name: string;
  description: string;
  parameters: JSONSchema;
  handler: (params: any) => Promise<any>;
}

// 智能体可调用预定义的函数
const agentFunctions: Record<string, AgentFunction> = {
  getCharacter: {
    id: 'getCharacter',
    name: '获取角色信息',
    description: '获取指定角色的详细信息',
    parameters: {
      type: 'object',
      properties: {
        characterName: { type: 'string' }
      },
      required: ['characterName']
    },
    handler: async (params) => {
      return await getCharacter(params.characterName);
    }
  }
};
```

---

**文档版本**: v1.0
**最后更新**: 2026-02-11
