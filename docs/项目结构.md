# Novel-IDE-Web 项目结构文档

本仓库当前处于 **Phase 1 可部署框架** 阶段：代码实现只包含最小可运行骨架；文档中若出现“完整模块目录树”，均视为后续规划，避免与实现混淆。
...
│
├── packages/
│   └── server-core/          # 共享服务端核心库（env/db/auth/redis/统一响应）
│
├── server/                   # NestJS（可选复杂能力服务）
│   ├── src/
...
│   └── server.Dockerfile
│
├── docs/                     # 设计与规划文档（会逐步对齐实现）
│   ├── spec-kit/             # Spec-Kit 方法论与模板
│   └── specs/                # 已交付/规划能力的规格文件
├── scripts/
├── pnpm-workspace.yaml
├── pnpm-lock.yaml
